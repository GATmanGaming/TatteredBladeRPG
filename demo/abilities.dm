...
Ability
	MeleeAttack
		...
		effect(mob/user)
			...
			target.noise('sound/hit-1.wav', frequency = rand(0.7, 1.3))
	...
	Cleave
		...
		effect(mob/user)
			...
			for(var/mob/target in user.all_melee_targets())
				...
				target.noise('sound/hit-2.wav', frequency = rand(0.7, 1.3))
	...
	Fireball
		...
		effect(mob/user)
			...
			user.noise('sound/fire.wav', frequency = rand(0.7, 1.3))
...
missile
	ShootArrow
		...
		hit(mob/m)
			...
			m.effect("arrow-hit")
			m.noise('sound/hit-1.wav', frequency = rand(0.7, 1.3))
